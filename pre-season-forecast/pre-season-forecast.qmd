---
title: "2026 Pre-Season Forecast"
format:
  gfm:
...
hindcast_year <- function(data, model, forecast_year) {
  ...
  prediction_interval <- prediction_fit + c(-2, 2) * qnorm(0.975) *
    prediction$se.fit[[1]]
  # Temporary override
  # prediction_interval <- prediction_fit + c(-3, 3)
...
```{r predictfifdj}
model_fifdj <- lm(fifdj ~ amatc, data = subset(yukon, year < forecast_year))
summary(model_fifdj)
...
```{r predictqdj}
model_qdj <- lm(qdj ~ amatc, data = subset(yukon, year < forecast_year))
summary(model_qdj)
...
```{r predictmdj}
model_mdj <- lm(mdj ~ amatc, data = subset(yukon, year < forecast_year))
summary(model_mdj)
...
hindcast_fifdj <- hindcast_model(yukon, "fifdj ~ amatc", hindcast_years)
hindcast_qdj <- hindcast_model(yukon, "qdj ~ amatc", hindcast_years)
hindcast_mdj <- hindcast_model(yukon, "mdj ~ amatc", hindcast_years)
